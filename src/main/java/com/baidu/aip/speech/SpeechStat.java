...
public class SpeechStat {
    ...
    public static StringBuilder formatResult(SpeechResult result) {
        StringBuilder sb = new StringBuilder();
        if (result.getErrNo() == -3005) {
            sb.append("【噪音】");
        } else if (result.getErrNo() != 0) {
            sb.append("【出错】");
        } else if (result.getType().equals(SpeechResult.TYPE_FIN_TEXT)) {
            sb.append("【一句话最终结果】");
        } else if (result.getType().equals(SpeechResult.TYPE_MID_TEXT)) {
            sb.append("【一句话临时结果】");
        }
